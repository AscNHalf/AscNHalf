...
INLINE bool IsCastedOnFriends(SpellEntry *sp)
{
...
INLINE bool IsCastedOnEnemies(SpellEntry *sp)
{
	for( int frloop = 0; frloop < 3; frloop++ )
	{
		switch (sp->EffectImplicitTargetA[frloop])
...
public:
	...
	INLINE SpellEntry *GetSpellProto() { return m_spellInfo; }
	...
	INLINE uint32 GetType() { return ( m_spellInfo->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }
...
/*		switch(m_spellInfo->NameHash)
	...
	INLINE static bool HasMechanic(SpellEntry * sp, uint32 MechanicsType)
	{
	...
	INLINE static uint32 GetMechanicOfEffect(SpellEntry * sp, uint32 i)
	{
	...
	INLINE static bool EffectAffectsSpell(SpellEntry* spellEffect, uint32 effectNum, SpellEntry* testSpell)
	{

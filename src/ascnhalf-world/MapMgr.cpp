		...
		cx >= _sizeX || cy >= _sizeY)
	{
		if( plObj != NULL )
		{
			if(plObj->GetBindMapId() != GetMapId())
			{
				...
				plObj->GetSession()->SystemMessage("Teleported you to your ascnhalf location as you ended up on the wrong map.");
				return;
			}
			else
			{
				obj->GetPositionV()->ChangeCoords(plObj->GetBindPositionX(),plObj->GetBindPositionY(),plObj->GetBindPositionZ(),0);
				plObj->GetSession()->SystemMessage("Teleported you to your ascnhalf location as you were out of the map boundaries.");
		...
		obj->GetPositionY() >= _maxY || obj->GetPositionY() <= _minY)
	{
		if(obj->IsPlayer())
		{
			...
			if(plr->GetBindMapId() != GetMapId())
			{
				...
				plr->GetSession()->SystemMessage("Teleported you to your ascnhalf location as you were out of the map boundaries.");
				return;
			}
			else
			{

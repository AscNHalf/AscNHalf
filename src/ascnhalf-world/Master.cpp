...
#define BANNER "AscNHalf r%u/%s-%s-%s :: World Server\n"
...
#ifdef WIN32
        static const char* default_config_file = "ascnhalf-world.conf";
        static const char* default_realm_config_file = "ascnhalf-realms.conf";
#else
        static const char* default_config_file = "../etc/ascnhalf-world.conf";
        static const char* default_realm_config_file = "../etc/ascnhalf-realms.conf";
#endif
...
bool Master::Run(int argc, char ** argv)
{
	...
	struct option longopts[] =
	{
		...
		{ "realmconf",			required_argument,		NULL,					'r'		},
		{ 0, 0, 0, 0 }
	};
	...
	if(geteuid() == 0 || getegid() == 0)
		Log.LargeErrorMessage( LARGERRORMESSAGE_WARNING, "You are running AscNHalf as root.", "This is not needed, and may be a possible security risk.", "It is advised to hit CTRL+C now and", "start as a non-privileged user.", NULL);
#endif
	...
	/* write pid file */
	FILE * fPid = fopen( "ascnhalf.pid", "w" );
	if( fPid )
	{
		uint32 pid;
#ifdef WIN32

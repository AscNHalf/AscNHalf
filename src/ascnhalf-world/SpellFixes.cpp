...
void DumpSpellsSQL()
{
	...
	for(uint32 x=0; x < (uint32)dbcSpell.GetNumRows(); x++)
	{
		...
		sprintf(query, "insert into spell(entry, name, description, attr, attrex, attrexb, "
				...
				"0x%x, 0x%x, %u, "
				"%u, %u, %u);",
				sp->Id, name.c_str(), desc.c_str(), sp->Attributes, sp->AttributesEx, sp->Flags3, 
			...
			case 48263://Frost Presence
				{
					...
					sp->Effect[1] = SPELL_EFFECT_APPLY_AURA;
					sp->Effect[2] = SPELL_EFFECT_APPLY_AURA;
					sp->EffectApplyAuraName[1] = SPELL_AURA_MOD_THREAT;
					sp->EffectApplyAuraName[2] = SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN;
					sp->EffectBasePoints[1] = 45;
					sp->EffectBasePoints[2] = -5;
				}break;
			case 61261://Frost Presence Stamina
				{
...
void ApplyNormalFixes()
{
	...
	for(uint32 x=0; x < cnt; x++)
	{
		...
		*/
		...
		if(!strcmp(sp->Name, "AscNHalf") || !strcmp(sp->Name, "Stuck") || !strcmp(sp->Name, "Astral Recall"))

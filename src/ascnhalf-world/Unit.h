...
public:
	...
	INLINE float GetSize() { return GetFloatValue(OBJECT_FIELD_SCALE_X) * GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS); }
	...
	INLINE void DelayPowerRegeneration(uint32 time) { m_P_regenTimer = time; if (!m_interruptedRegenTime) m_interruptedRegenTime = 2000; }
	void DeMorph();
	...
	// Stealth  
	INLINE int32 GetStealthLevel() { return (m_stealthLevel + (getLevel() * 5)); }
	INLINE int32 GetStealthDetectBonus() { return m_stealthDetectBonus; }
	INLINE void SetStealth(uint32 id) { m_stealth = id; }
	INLINE bool InStealth() { return (m_stealth!=0 ? true : false); }
	float detectRange;

	// Invisibility
	INLINE void SetInvisibility(uint32 id) { m_invisibility = id; }
	INLINE bool IsInvisible() { return (m_invisible!=0 ? true : false); }
	...
	uint32 m_addDmgOnce;
	Creature* m_SummonSlots[NUM_SUMMON_SLOTS];
	...
	INLINE void SetOnMeleeSpell(uint32 spell, uint8 cast_number ) { m_meleespell = spell; m_meleespell_cn = cast_number; }
	...
	INLINE uint32 IsFeared() { return m_fearmodifiers; }
	...
	int32 APvModifier;
	uint64 stalkedby;
	uint32 dispels[NUM_DISPEL_TYPES];
	...
	INLINE uint32 computeOverkill(uint32 damage)
	{
	...
	INLINE int32 GetHealthPct() { return (int32)(GetUInt32Value(UNIT_FIELD_HEALTH) * 100 / std::max(1, (int32)GetUInt32Value(UNIT_FIELD_MAXHEALTH))); }
	...
	INLINE int32 GetManaPct() { return (int32)(GetUInt32Value(UNIT_FIELD_POWER1) * 100 / std::max(1, (int32)GetUInt32Value(UNIT_FIELD_MAXPOWER1))); }
	...
	INLINE int8 GetSeatID() { return m_inVehicleSeatId; }
	...
	int32 CreatureAttackPowerMod[NUM_UNIT_TYPE];
	int32 CreatureRangedAttackPowerMod[NUM_UNIT_TYPE];
	float AOEDmgMod;
	...
	INLINE bool ClassMaskAffect(uint32 *classmask, SpellEntry* testSpell)
	{

...
public:
	...
	INLINE void SetSheatheForAttackType(uint8 type) { SetByte(UNIT_FIELD_BYTES_2, 0, type); }
	...
	Unit* GetSummonOwner();
	INLINE int32 GetSummonSlot() { return SummonSlot; }
	INLINE bool IsSummon() { return SummonOwner && SummonSlot > -1; }
	INLINE void SetSummonOwnerSlot(uint64 ownerguid, int8 slot) { SummonOwner = ownerguid; SummonSlot = slot;}

	//Special summons, Totems
	INLINE bool IsTotem() { return Totem;}
	INLINE void SetTotem(bool totem) { Totem = totem;}
	...
	INLINE CreatureProtoHeroic * GetProtoHeroic() { return proto_heroic; }
...
public:
	...
	INLINE uint32 GetProtoItemDisplayId(uint8 i) { return GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i); }

...
struct BattlemasterListEntry
{
    ...
    //uint32 unk1;                                          // 10 (0 or 1)
    char*   name[16];                                       // 10-26
    //uint32 unk2                                           // 27 unknown
    //uint32 unk3                                           // 28 unused
	//uint32 unk3											// 29 unused
    uint32 minlevel;                                        // 30 new 3.1
	uint32 maxlevel;										// 31 new 3.3.3 (80)
};
...
{
	/*1*/uint32 id;
	/*2*/char* name_internal;
	/*3*/uint32 map_type;
	/*4*/uint32 is_pvp_zone;
	/*5*/char* real_name;
	/*6*/uint32 linked_zone;		// common zone for instance and continent map
	/*7*/char* hordeIntro;		// text for PvP Zones
	/*8*/char* allianceIntro;	// text for PvP Zones
	/*9*/uint32 multimap_id;		// seems to be 0 for all test maps.
	/*10*/uint32 unk;				// Unknown
	/*11*/int32 parent_map;		// map_id of parent map
	/*12*/float start_x;			// enter x coordinate (if exist single entry)
	/*13*/float start_y;			// enter y coordinate (if exist single entry)
	/*14*/uint32 unk2;			// Unknown
	/*15*/uint32 addon;			// 0-original maps, 1-tbc addon, 2-wotlk addon
	/*16*/uint32 unk3;			// Unknown
	/*17*/uint32 maxPlayers;		// max players
...
INLINE float GetScale(CreatureDisplayInfo *Scale)
{
	return Scale->Scale;
}

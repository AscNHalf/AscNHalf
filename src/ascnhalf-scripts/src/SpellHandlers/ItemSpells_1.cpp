...
bool StormHammer(uint32 i, Spell * pSpell)
{
	Unit *  target = pSpell->GetUnitTarget();
	if(target->IsPlayer() == true)		// can't use it on players
	{
		return true;
	}
	
	if(pSpell->p_caster->GetMapId() == 571 && pSpell->p_caster->GetZoneId() == 67)		// can use it in storm peaks
	{
		return false;
	}
...
bool SturdyCrates(uint32 i, Spell * pSpell)
{
	if(!pSpell->p_caster) return true;
	...
	GameObject* crate = pSpell->p_caster->GetMapMgr()->GetInterface()->SpawnGameObject(190510, SSX, SSY, SSZ, SSO, 0, 0, 0);
	crate->SetUInt32Value(GAMEOBJECT_FLAGS, 3);		// make it lootable
	crate->PushToWorld(pSpell->p_caster->GetMapMgr());
	crate->ExpireAndDelete(10000);
...
void SetupItemSpells_1(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(56448, &StormHammer);				// Battered Storm Hammer
	mgr->register_dummy_spell(47431, &SturdyCrates);			// Capture Jormungar Spawn
